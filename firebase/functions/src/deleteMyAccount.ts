...
export const deleteMyAccount = onCall(async (request) => {
  ...
  for (const member of memberships.docs) {
    ...
    await deleteByQuery(db.collection(`families/${familyId}/parentInvites`).where("createdByUid", "==", uid));
    await deleteByQuery(db.collection(`families/${familyId}/commands`).where("createdByUid", "==", uid));

    if (role === "child") {
      await deleteByQuery(db.collection(`families/${familyId}/commands`).where("targetDeviceUid", "==", uid));
    }
  }
