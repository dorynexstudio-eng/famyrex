...
class CommunicationSignalDetectorTest {
    ...
    fun `personal information plus secret is contextual grooming signal`() {
        val summary = CommunicationSignalDetector.detect(
            listOf(
                obs(0, "dime dónde vives"),
                obs(5 * 60 * 1000L, "no se lo digas a nadie")
            )
        )
    ...
    fun `sexual request plus secrecy can reach high risk`() {
        val summary = CommunicationSignalDetector.detect(
            listOf(
                obs(0, "manda nudes"),
                obs(2 * 60 * 1000L, "es nuestro secreto")
            )
        )
    ...
    fun `repeated self harm expressions become high signal`() {
        val summary = CommunicationSignalDetector.detect(
            listOf(
                obs(0, "no quiero vivir"),
                obs(10 * 60 * 1000L, "me quiero morir")
            )
        )
    ...
    fun `generic word threat does not trigger`() {
        val summary = CommunicationSignalDetector.detect(
            listOf(obs(0, "he leído una noticia sobre una amenaza"))
        )
