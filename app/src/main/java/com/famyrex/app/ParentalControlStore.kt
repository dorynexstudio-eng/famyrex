...
class ParentalControlStore(context: Context) {
    ...
    fun save(config: ParentalControlConfig) { persist(config, waitForDisk = false) }

    /** Persists the complete policy before returning, for revisioned sync transactions. */
    fun saveBlocking(config: ParentalControlConfig): Boolean = persist(config, waitForDisk = true)
    ...
    companion object { private const val KEY_CONFIG = "config" }
}

internal fun parseParentalControlConfig(raw: String): ParentalControlConfig = runCatching {
    val root = JSONObject(raw)
    val screenTime = root.optJSONObject("screenTime")?.let { item -> runCatching {
