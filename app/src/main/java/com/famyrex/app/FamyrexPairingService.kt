...
class FamyrexPairingService(context: Context) {
    ...
    fun createInvite(
        ...
        functions().getHttpsCallable("createPairingInvite")
            ...
            .addOnSuccessListener { result ->
                ...
                if (code.isNullOrBlank() || token.isNullOrBlank() || expiresAtMs == null) onError("Firebase devolvió una invitación incompleta.")
                else { showChildInvitationQr(childLabel, code, token); onSuccess(code, token, expiresAtMs) }
            }.addOnFailureListener { onError(it.toUserMessage()) }
    }
    ...
    private fun showChildInvitationQr(childLabel: String, code: String, token: String) {
        ...
        val link = Uri.Builder().scheme("famyrex").authority("join")
            .appendQueryParameter("code", code).appendQueryParameter("token", token).appendQueryParameter("label", safeLabel).build().toString()
    ...
    fun redeemCode(
        code: String, token: String, childLabel: String, famyrexMemberId: String?, famyrexDeviceId: String,
        ...
        fun redeem() {
            ...
            functions().getHttpsCallable("redeemPairingCode").call(payload)
                .addOnSuccessListener { result ->
                    val data = result.data as? Map<*, *>
                    ...
                    if (familyId.isNullOrBlank() || childUid.isNullOrBlank() || memberId.isNullOrBlank() || deviceId.isNullOrBlank()) onError("Firebase devolvió una vinculación incompleta.")
                    else onSuccess(familyId, childUid, memberId, deviceId)
                }.addOnFailureListener { onError(it.toUserMessage()) }
        }
        val current = auth.currentUser
        if (current != null && current.providerData.none { it.providerId == "anonymous" }) { onError("Este dispositivo tiene una sesión de usuario incompatible con el modo infantil."); return }
        if (current != null) redeem() else auth.signInAnonymously().addOnSuccessListener { redeem() }.addOnFailureListener { onError(it.toUserMessage()) }
    }
    ...
    private fun Throwable.toUserMessage(): String {
        ...
        return firebase?.message?.takeIf { it.isNotBlank() } ?: message?.takeIf { it.isNotBlank() } ?: "No se pudo completar la operación familiar."
    }
}