...
@Composable
fun PremiumLocationScreen(context: Context, zones: List<GeoZone>, onZonesChange: (List<GeoZone>) -> Unit, modifier: Modifier = Modifier) {
    ...
    PremiumScreenScaffold(modifier, Icons.Default.LocationOn, "Mapa", "Ubicación familiar y lugares seguros") {
        PremiumInfoCard(Icons.Default.LocationOn, if (foreground) "Ubicación disponible" else "Ubicación desactivada", if (foreground) { if (background) "Famyrex tiene el permiso necesario para actualizar ubicación en segundo plano." else "La ubicación funciona mientras la app está en uso. Para geozonas y actualizaciones en segundo plano hace falta un permiso adicional de Android." } else "Activa la ubicación solo si quieres utilizar esta función. Famyrex la utiliza para geozonas y protección familiar.", onClick = { if (!foreground) foregroundLauncher.launch(arrayOf(Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION)) else details = !details }, actionLabel = if (!foreground) "Activar ubicación" else if (details) "Ocultar gestión" else "Gestionar ubicación")
        if (foreground && Build.VERSION.SDK_INT >= 29 && !background) PremiumInfoCard(Icons.Default.Security, "Ubicación en segundo plano", "Esta función es opcional. Permite mantener geozonas y recibir cambios de ubicación cuando Famyrex no está abierta. Android mostrará su propia pantalla de permisos.", onClick = { disclosure = true }, actionLabel = "Activar segundo plano")
        ...
        if (disclosure) Card(Modifier.fillMaxWidth(), shape = androidx.compose.foundation.shape.RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
            Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Button(onClick = { disclosure = false; if (Build.VERSION.SDK_INT >= 29) backgroundLauncher.launch(Manifest.permission.ACCESS_BACKGROUND_LOCATION) }, modifier = Modifier.weight(1f)) { Text("Continuar") }
                }
            }
        }
...
fun PremiumAssistantScreen(context: Context, modifier: Modifier = Modifier) {
    ...
    PremiumScreenScaffold(modifier, Icons.Default.Security, "Asistente", "Entiende lo que ocurre y decide qué hacer después") {
        ...
        if (open) Card(Modifier.fillMaxWidth(), shape = androidx.compose.foundation.shape.RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) { FamilyAssistantScreen(context, Modifier.fillMaxWidth()) }
    }
}
...
@Composable
private fun PremiumScreenScaffold(modifier: Modifier, icon: androidx.compose.ui.graphics.vector.ImageVector, title: String, subtitle: String, content: @Composable ColumnScope.() -> Unit) {
    LazyColumn(modifier.padding(horizontal = 18.dp, vertical = 10.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        item {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                Surface(shape = androidx.compose.foundation.shape.RoundedCornerShape(18.dp), color = MaterialTheme.colorScheme.primaryContainer) { Icon(icon, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(12.dp).size(28.dp)) }
        ...
        item { Text("Famyrex prioriza señales y contexto. Las funciones de protección dependen de los permisos y accesos que la familia decida activar.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(bottom = 8.dp)) }
    }
}

@Composable
private fun PremiumInfoCard(icon: androidx.compose.ui.graphics.vector.ImageVector, title: String, text: String, onClick: (() -> Unit)? = null, actionLabel: String? = null) {
    Card(onClick = onClick ?: {}, enabled = onClick != null, modifier = Modifier.fillMaxWidth(), shape = androidx.compose.foundation.shape.RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) {
        Column(Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Surface(shape = androidx.compose.foundation.shape.RoundedCornerShape(14.dp), color = MaterialTheme.colorScheme.primaryContainer) { Icon(icon, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(10.dp).size(24.dp)) }
                Spacer(Modifier.size(12.dp)); Text(title, style = MaterialTheme.typography.titleLarge)
            }
...
private fun PremiumPersonCard(name: String, role: String, icon: androidx.compose.ui.graphics.vector.ImageVector) {
    Card(Modifier.fillMaxWidth(), shape = androidx.compose.foundation.shape.RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainerLow)) {
        Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Spacer(Modifier.size(14.dp)); Column { Text(name, style = MaterialTheme.typography.titleLarge); Text(role, color = MaterialTheme.colorScheme.onSurfaceVariant) }
        }
    }
}

private fun hasForegroundLocation(context: Context): Boolean = ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED

private fun hasBackgroundLocation(context: Context): Boolean = Build.VERSION.SDK_INT < 29 || ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_BACKGROUND_LOCATION) == PackageManager.PERMISSION_GRANTED
