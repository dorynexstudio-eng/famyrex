...
enum class FamilyIntelligenceEvidenceType { STATUS, COMMUNICATION, TREND, ANOMALY, DATA_GAP }
...
object FamilyIntelligenceEvidenceBuilder {
    fun fromSummary(summary: FamilyIntelligenceSummary): List<FamilyIntelligenceEvidence> = buildList {
        when (summary.parentalStatus) {
            ParentalStatus.RED -> add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.STATUS, "Límite configurado alcanzado o superado.", "Estado 🔴 Acción necesaria.", "Revisar el límite y el uso de hoy."))
            ParentalStatus.ORANGE -> add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.STATUS, "Uso cercano a un límite configurado.", "Estado 🟠 Revisar.", "Revisar cómo va el día."))
            ParentalStatus.GREEN -> add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.STATUS, "Los datos disponibles no muestran un límite alcanzado.", "Estado 🟢 En orden.", "Continuar observando."))
            ParentalStatus.WHITE -> add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.DATA_GAP, "Faltan datos o permisos suficientes.", "Estado ⚪ Sin datos suficientes.", "Completar los permisos o datos necesarios."))
        }
        if (summary.communicationAlertCount > 0) add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.COMMUNICATION, "Hay ${summary.communicationAlertCount} señal${if (summary.communicationAlertCount == 1) "" else "es"} de comunicación pendiente${if (summary.communicationAlertCount == 1) "" else "s"} de revisión.", "La comunicación requiere revisión, sin atribuir intención.", "Revisar la señal y su contexto."))
        if (!summary.protectionReady) add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.DATA_GAP, "La protección no está completamente preparada.", "La valoración puede ser incompleta.", "Revisar acceso de uso y guardia parental."))
    }

    fun fromIncidents(incidents: List<CommunicationRiskIncident>): List<FamilyIntelligenceEvidence> = incidents.asSequence()
    ...
    fun fromTrend(trend: FamilyUsageTrend?): List<FamilyIntelligenceEvidence> {
        ...
        when (trend.direction) {
            FamilyUsageTrendDirection.INCREASING -> evidence += FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.TREND, "El uso de hoy está por encima de la referencia reciente.", "Hay una tendencia creciente de uso.", "Observar la evolución; la variación no demuestra una causa.")
            FamilyUsageTrendDirection.DECREASING -> evidence += FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.TREND, "El uso de hoy está por debajo de la referencia reciente.", "Hay una tendencia decreciente de uso.", "Continuar observando la evolución.")
            FamilyUsageTrendDirection.STABLE -> Unit
            FamilyUsageTrendDirection.INSUFFICIENT_DATA -> evidence += FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.DATA_GAP, "Todavía no hay suficientes datos históricos para comparar.", "La tendencia no puede valorarse todavía.", "Seguir recopilando datos de uso antes de extraer conclusiones.")
        }
    ...
    fun build(summary: FamilyIntelligenceSummary, trend: FamilyUsageTrend?, incidents: List<CommunicationRiskIncident> = emptyList()): List<FamilyIntelligenceEvidence> = fromSummary(summary) + fromIncidents(incidents) + fromTrend(trend)
    ...
    fun explain(summary: FamilyIntelligenceSummary, trend: FamilyUsageTrend?, incidents: List<CommunicationRiskIncident> = emptyList()): String {
        ...
        evidence.firstOrNull { it.type == FamilyIntelligenceEvidenceType.ANOMALY }?.let { return "${it.signal} ${it.action} Sin asumir su causa. Es una variación que conviene observar." }
        evidence.firstOrNull { it.type == FamilyIntelligenceEvidenceType.DATA_GAP }?.let { return "${it.signal} ${it.conclusion} ${it.action}" }
