...
object FamilyIntelligenceEvidenceBuilder {
    fun fromSummary(summary: FamilyIntelligenceSummary): List<FamilyIntelligenceEvidence> = buildList {
        ...
        if (summary.communicationAlertCount > 0) {
            add(FamilyIntelligenceEvidence(
        ...
        if (!summary.protectionReady) {
            add(FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.DATA_GAP, "La protección no está completamente preparada.", "La valoración puede ser incompleta.", "Revisar acceso de uso y guardia parental."))
        }
    }
    ...
    fun fromTrend(trend: FamilyUsageTrend?): List<FamilyIntelligenceEvidence> {
        ...
        trend.anomaly?.let { anomaly ->
            ...
            evidence += FamilyIntelligenceEvidence(FamilyIntelligenceEvidenceType.ANOMALY, "El uso está un ${anomaly.deviationPercent}% $direction de la referencia reciente.", "Se observa una variación estadística relevante.", "Revisar el contexto; esta variación no demuestra una causa.")
        }
        return evidence
    }

    fun build(summary: FamilyIntelligenceSummary, trend: FamilyUsageTrend?, incidents: List<CommunicationRiskIncident> = emptyList()): List<FamilyIntelligenceEvidence> =
